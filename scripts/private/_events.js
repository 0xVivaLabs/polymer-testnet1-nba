...
function filterChannelEvents(portAddress) {
  ...
  return (
    areAddressesEqual(portAddress, config.createChannel["srcAddr"]) ||
    areAddressesEqual(portAddress, config.createChannel["dstAddr"])
  );
}
...
function listenForIbcChannelEvents(network, source, dispatcher) {
  ...
  dispatcher.on(
    ...
    (
      ...
      const counterpartyChannelIdString = hre.ethers.decodeBytes32String(
        counterpartyChannelId
      );
...
function filterPacketEvents(portAddress, network) {
  ...
  return (
    areAddressesEqual(portAddress, sendPacketConfig[`${network}`].portAddr) ||
    areAddressesEqual(portAddress, ucHandlerAddr)
  );
}
...
function listenForIbcPacketEvents(network, dispatcher) {
  ...
  dispatcher.on(
    ...
    (
      ...
      const txHash = event.log.transactionHash;
      const sourceChannelIdString =
        hre.ethers.decodeBytes32String(sourceChannelId);
      const url = `${explorerUrl}tx/${txHash}`;
  ...
  dispatcher.on(
    ...
    (writerPortAddress, writerChannelId, sequence, ackPacket, event) => {
      const txHash = event.log.transactionHash;
      const writerChannelIdString =
        hre.ethers.decodeBytes32String(writerChannelId);
      const url = `${explorerUrl}tx/${txHash}`;
